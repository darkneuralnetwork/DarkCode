...
const cmdItems = [
  ...
  { group: "Navigate", icon: "📐", label: "Blueprint (Plan & Workflow)", hint: "blueprint plan workflow tasks decisions cascade consensus debate", action: () => switchTab("blueprint") },
  { group: "Navigate", icon: "📡", label: "Telemetry", hint: "telemetry live events monitor charts tokens cost replay audit", action: () => switchTab("telemetry") },
  { group: "Navigate", icon: "🧠", label: "Knowledge", hint: "knowledge memory recall tools mcp strategies", action: () => switchTab("knowledge") },
  { group: "Navigate", icon: "📁", label: "Projects", hint: "projects changes rollback diff", action: () => switchTab("projects") },
